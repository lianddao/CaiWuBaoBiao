...
import com.hzsh.zhks.mapper.KaiShiXiaoShouShouRuMapper;
 ...
 */
@Service
public class KaiShiXiaoShouShouRuServiceImpl extends ServiceImpl<KaiShiXiaoShouShouRuMapper, KaiShiXiaoShouShouRu> implements KaiShiXiaoShouShouRuService {
    ...
    public List<KaiShiXiaoShouShouRu> importKaiShiXiaoShouShouRu(MultipartFile file) {
        // 1. 从文件中提取数据
        List<KaiShiXiaoShouShouRu> list = getExcelDate(file);
        ...
        for (KaiShiXiaoShouShouRu one : list) {
            insertOrUpdate(one);
        }
    ...
    @Override
    public List<KaiShiXiaoShouShouRu> defaultList(String month) {
        LambdaQueryWrapper<KaiShiXiaoShouShouRu> queryWrapper = new LambdaQueryWrapper<>();
        queryWrapper
                .eq(KaiShiXiaoShouShouRu::getMonth, DateTime.parse(month).toDate())
                .orderByAsc(KaiShiXiaoShouShouRu::getSort);
        List<KaiShiXiaoShouShouRu> list = baseMapper.selectList(queryWrapper);
    ...
    private List<KaiShiXiaoShouShouRu> getExcelDate(MultipartFile file) {
        // 1. 打开文件
        Workbook wb;
        ...
        List<KaiShiXiaoShouShouRu> fetchDataList = new ArrayList<>();
        ...
        try {
            for (Row row : sheet) {
                ...
                // 1. 基础数据
                KaiShiXiaoShouShouRu one = new KaiShiXiaoShouShouRu();
                one.month = DateTime.parse(month).toDate();
                one.code = PeiZhi.CATEGORY_开氏_销售收入 + new DecimalFormat("000000").format(codeNumber++);
                one.category = "开氏" + CATEGORY;
                one.name = name.trim();
                one.消费税率 = excelUtil.getDouble(row, 消费税率CellIndex);
                one.本月销量 = excelUtil.getDouble(row, 本月销量CellIndex);
                one.含税价 = excelUtil.getDouble(row, 含税价CellIndex);
                one.不含税价 = excelUtil.getDouble(row, 不含税价CellIndex);
                one.收入 = excelUtil.getDouble(row, 收入CellIndex);
                one.裸税价 = excelUtil.getDouble(row, 裸税价CellIndex);
    ...
    public KaiShiXiaoShouShouRu getUnique(String month, String code) {
        ...
        queryWrapper
                .eq(KaiShiXiaoShouShouRu::getMonth, DateTime.parse(month).toDate())
                .eq(KaiShiXiaoShouShouRu::getCode, code);
    ...
    public int edit(String name, String value, Long pk) {
        if (StringUtils.isEmpty(value)) {
            ...
            Field field = Arrays.stream(KaiShiXiaoShouShouRu.class.getDeclaredFields()).filter(i -> i.getName().equals(name)).findFirst().get();
            TableField tableField = field.getAnnotation(TableField.class);
            String 数据库列名 = tableField == null ? name : tableField.value();
            UpdateWrapper<KaiShiXiaoShouShouRu> updateWrapper = new UpdateWrapper<>();
            updateWrapper
                    .eq("id", pk)
                    .set(数据库列名, value);
            ...
            Map map = new HashMap();
            map.put(name, value);
            KaiShiXiaoShouShouRu one = JSON.parseObject(JSON.toJSONString(map), KaiShiXiaoShouShouRu.class);
    ...
    public HSSFSheet 返回导数模板(HSSFWorkbook workbook) {
        ...
        HSSFCell
                ...
                消费税率HeaderCell = 表头行.createCell(1),
                本月销量HeaderCell = 表头行.createCell(2),
                含税价HeaderCell = 表头行.createCell(3),
                不含税价HeaderCell = 表头行.createCell(4),
                收入HeaderCell = 表头行.createCell(5),
                裸税价HeaderCell = 表头行.createCell(6),
                编码HeaderCell = 表头行.createCell(7);
        项目HeaderCell.setCellValue("项目");
        消费税率HeaderCell.setCellValue("消费税率");
        本月销量HeaderCell.setCellValue("本月销量(吨)");
        含税价HeaderCell.setCellValue("含税价(元/吨)");
        不含税价HeaderCell.setCellValue("不含税价(元/吨)");
        收入HeaderCell.setCellValue("收入(万元)");
        裸税价HeaderCell.setCellValue("裸税价(元/吨)");
        ...
        for (PeiZhi peiZhi : peiZhiList) {
            ...
            HSSFCell 消费税率Cell = row.createCell(1);

            HSSFCell 本月销量Cell = row.createCell(2);

            HSSFCell 含税价Cell = row.createCell(3);

            HSSFCell 不含税价Cell = row.createCell(4);

            HSSFCell 收入Cell = row.createCell(5);

            HSSFCell 裸税价Cell = row.createCell(6);
    ...
    public int 从导数模板导入数据(MultipartFile file) {
        ...
        try {
            for (Row row : sheet) {
                ...
                one.category = "开氏" + CATEGORY;
                one.name = name.trim();
                one.消费税率 = excelUtil.getDouble(row, 消费税率Cell);
                one.本月销量 = excelUtil.getDouble(row, 本月销量Cell);
                one.含税价 = excelUtil.getDouble(row, 含税价Cell);
                one.不含税价 = excelUtil.getDouble(row, 不含税价Cell);
                one.收入 = excelUtil.getDouble(row, 收入Cell);
                one.裸税价 = excelUtil.getDouble(row, 裸税价Cell);
        ...
        // 7. 新增或修改数据
        for (KaiShiXiaoShouShouRu one : fetchDataList) {
            insertOrUpdate(one);
        }
    ...
    @Override
    public boolean insertOrUpdate(KaiShiXiaoShouShouRu one) {
        String month = one.getMonthString();

        KaiShiXiaoShouShouRu dbOne = getUnique(month, one.code);
